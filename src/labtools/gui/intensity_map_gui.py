"""HORIBA-style PyQt6 GUI for resumable intensity maps and ITC4000 control."""
...
from PyQt6.QtWidgets import (
    QApplication, QAbstractSpinBox, QCheckBox, QComboBox, QDoubleSpinBox, QFileDialog,
    QFormLayout, QFrame, QGridLayout, QGroupBox, QHBoxLayout, QLabel,
    QLineEdit, QMainWindow, QMessageBox, QProgressBar, QPushButton,
    QScrollArea, QSizePolicy, QSpinBox, QSplitter, QToolButton, QVBoxLayout, QWidget,
)
...
class IntensityMapWindow(QMainWindow):
    """Main window for count maps, checkpoint resume, and laser control."""
    ...
    def __init__(self) -> None:
        ...
        subtitle = QLabel(
            "Configure the scanning mirror and Time Controller, acquire resumable maps, "
            "and control the excitation laser"
        )
    ...
    def _apply_application_style(self) -> None:
        self.setStyleSheet("""
            ...
            QGroupBox {
                margin-top: 11px; padding: 10px 8px 8px 8px;
                font-weight: 650; color: #243b53;
            }
            ...
            QPushButton#primaryButton {
                min-height: 39px; background: #0b6fa4; border: 1px solid #0b6fa4;
                color: white; font-weight: 700;
            }
            QPushButton#primaryButton:hover { background: #095d8a; }
            QPushButton#dangerButton { color: #b42318; border-color: #f0b4ae; }
            QCheckBox { spacing: 7px; }
            QProgressBar { min-height: 18px; border: 1px solid #bcccdc; border-radius: 5px; text-align: center; }
    ...
    def _style_empty_axes(self) -> None:
        ...
        self.image = self.axes.imshow(
            np.full((2, 2), np.nan), origin="lower", aspect="auto",
            interpolation="nearest", cmap="inferno", extent=(0, 1, 0, 1)
        )
    ...
    def _build_map_controls(self, root: QVBoxLayout) -> None:
        ...
        self.channel_1 = QComboBox(); self.channel_1.addItems(["1", "2", "3", "4"])
        self.threshold_1 = self._threshold_box(0.1)
        self.edge_1 = QComboBox(); self.edge_1.addItems(["Rising", "Falling"])
        self.use_second = QCheckBox("Acquire a second channel and plot the sum")
        self.channel_2 = QComboBox(); self.channel_2.addItems(["1", "2", "3", "4"])
        self.channel_2.setCurrentText("2")
        self.threshold_2 = self._threshold_box(0.1)
        self.edge_2 = QComboBox(); self.edge_2.addItems(["Rising", "Falling"])
        self.second_channel_widget = self._channel_settings_widget(
            self.channel_2, self.threshold_2, self.edge_2
        )
        ...
        tc_form.addRow(
            "Primary channel",
            self._channel_settings_widget(self.channel_1, self.threshold_1, self.edge_1),
        )
        ...
        hint = QLabel("Each selected channel is saved separately; two-channel maps display their summed count rate.")
        hint.setObjectName("hintText"); hint.setWordWrap(True)
        tc_form.addRow("", hint)
        self.time_controller_group = tc
        root.addWidget(tc)
        self.time_controller_toggle = self._make_collapsible(
            tc,
        ...
        for box in (
            self.x_start, self.x_stop, self.x_points,
            self.y_start, self.y_stop, self.y_points,
            self.settle, self.integration,
        ):
    ...
    def _build_laser_controls(self, root: QVBoxLayout) -> None:
        ...
        form = QFormLayout(self.laser_group); form.setVerticalSpacing(9)
        self.laser_address = QLineEdit(ITC4000.DEFAULT_ADDRESS)
        self.laser_current = QDoubleSpinBox(); self.laser_current.setRange(0.0, MAX_LASER_CURRENT_A)
        self.laser_current.setDecimals(3); self.laser_current.setSingleStep(0.001)
        self.laser_current.setValue(DEFAULT_LASER_CURRENT_A); self.laser_current.setSuffix(" A")
        self.set_laser_indicator(False)
        form.addRow("VISA address", self.laser_address)
        form.addRow("Drive current", self.laser_current)
        self.laser_connect = QPushButton("Connect"); self.laser_set = QPushButton("Apply current")
        self.laser_on = QPushButton("Output ON"); self.laser_off = QPushButton("Output OFF")
        grid = QGridLayout(); grid.addWidget(self.laser_connect, 0, 0); grid.addWidget(self.laser_set, 0, 1)
        grid.addWidget(self.laser_on, 1, 0); grid.addWidget(self.laser_off, 1, 1)
        form.addRow(grid); root.addWidget(self.laser_group)
    ...
    def _sync_second_channel_visibility(self, checked: bool | None = None) -> None:
        """Show the complete second-channel row only when requested."""
    ...
    def _build_persistent_acquisition_controls(self, root: QVBoxLayout) -> None:
        """Add estimate and map controls that remain visible at all times."""
        panel = QFrame()
    ...
    def _make_collapsible(
        ...
        *,
        ...
        """Add a stable arrow header to a form-layout section.

        Qt 6 can hide complete QFormLayout rows. This preserves label/field
        relationships and leaves the header row visible when collapsed.
        """
        title = group.title()
        group.setTitle("")
    ...
    @staticmethod
    def _channel_settings_widget(channel, threshold, edge) -> QWidget:
        widget = QWidget()
    ...
    @staticmethod
    def _axis_controls(start: float, stop: float, points: int):
    ...
    def _channels(self) -> tuple[int, ...]:
        ...
        return (first, int(self.channel_2.currentText())) if self.use_second.isChecked() else (first,)

    def _thresholds(self) -> tuple[float, ...]:
        first = float(self.threshold_1.value())
        return (first, float(self.threshold_2.value())) if self.use_second.isChecked() else (first,)

    def _edges(self) -> tuple[str, ...]:
        first = self.edge_1.currentText().lower()
        return (first, self.edge_2.currentText().lower()) if self.use_second.isChecked() else (first,)

    def _config(self) -> IntensityMapConfig:
        return IntensityMapConfig(
            x_start_v=self.x_start.value(), x_stop_v=self.x_stop.value(), x_points=self.x_points.value(),
            y_start_v=self.y_start.value(), y_stop_v=self.y_stop.value(), y_points=self.y_points.value(),
            integration_time_s=self.integration.value(), channels=self._channels(),
            channel_thresholds_v=self._thresholds(), channel_edges=self._edges(),
            settle_time_s=self.settle.value(), time_controller_address=self.address.text().strip(),
            serpentine=self.serpentine.isChecked(), output_root=Path(self.output_root.text().strip()),
            save_results=self.save_results.isChecked(),
        )

    def _apply_config(self, config: IntensityMapConfig) -> None:
        for control, value in ((self.x_start, config.x_start_v), (self.x_stop, config.x_stop_v),
                               (self.y_start, config.y_start_v), (self.y_stop, config.y_stop_v),
                               (self.integration, config.integration_time_s), (self.settle, config.settle_time_s)):
            control.setValue(value)
        self.x_points.setValue(config.x_points); self.y_points.setValue(config.y_points)
        self.address.setText(config.time_controller_address); self.channel_1.setCurrentText(str(config.channels[0]))
        self.threshold_1.setValue(config.channel_thresholds_v[0]); self.edge_1.setCurrentText(config.channel_edges[0].title())
        ...
        self.serpentine.setChecked(config.serpentine); self.output_root.setText(str(config.output_root))
        self.save_results.setChecked(config.save_results)

    def _update_estimate(self) -> None:
        try: seconds = estimated_minimum_duration_s(self._config())
        ...
        self.estimate.setText(f"Estimated minimum: {seconds:.1f} s plus overhead | {save_text}")

    def _choose_output(self) -> None:
        selected = QFileDialog.getExistingDirectory(self, "Select map output folder", self.output_root.text())
        if selected: self.output_root.setText(selected)

    def start_new(self) -> None:
        self._resume_directory = None; self._start_worker(self._config())

    def choose_resume(self) -> None:
        directory = QFileDialog.getExistingDirectory(self, "Select incomplete map directory", self.output_root.text())
        if not directory: return
        try: config = config_from_resume_directory(directory)
        except Exception as exc: QMessageBox.critical(self, "Cannot resume map", str(exc)); return
        self._resume_directory = Path(directory); self._apply_config(config); self._start_worker(config)

    def _start_worker(self, config: IntensityMapConfig) -> None:
        try: config.validate()
        except ValueError as exc: QMessageBox.warning(self, "Invalid map settings", str(exc)); return
        if len(config.channels) == 2 and config.channels[0] == config.channels[1]:
            QMessageBox.warning(self, "Invalid channels", "Select two different Time Controller channels."); return
        self.start_button.setEnabled(False); self.resume_button.setEnabled(False); self.stop_button.setEnabled(True)
        self.progress.setValue(0); self.set_status("Starting acquisition...")
        self._thread = QThread(self); self._worker = MapWorker(config, self._resume_directory)
        self._worker.moveToThread(self._thread); self._thread.started.connect(self._worker.run)
        self._worker.progress.connect(self._on_progress); self._worker.finished.connect(self._on_finished)
        self._worker.failed.connect(self._on_failed); self._worker.finished.connect(self._thread.quit)
        self._worker.failed.connect(self._thread.quit); self._thread.finished.connect(self._cleanup_worker)
        self._thread.start()

    def request_stop(self) -> None:
        self.stop_button.setEnabled(False); self.set_status("Stopping after the current point and saving checkpoint...")
        if self._worker is not None: self._worker.cancel()

    @pyqtSlot(object)
    def _on_progress(self, snapshot: IntensityMapSnapshot) -> None:
        self.progress.setValue(int(100 * snapshot.points_complete / snapshot.total_points))
        title = f"Channels {' + '.join(map(str, snapshot.channels))}: {snapshot.points_complete}/{snapshot.total_points}"
        if self.image.get_array().shape != snapshot.summed_count_rate_cps.shape:
            self.colourbar.remove(); self.axes.clear()
            self.image = self.axes.imshow(snapshot.summed_count_rate_cps, origin="lower",
                extent=(float(snapshot.x_values_v.min()), float(snapshot.x_values_v.max()),
                        float(snapshot.y_values_v.min()), float(snapshot.y_values_v.max())),
                aspect="auto", interpolation="nearest", cmap="inferno")
            self.axes.set_xlabel("Mirror X voltage (V)"); self.axes.set_ylabel("Mirror Y voltage (V)")
            self.colourbar = self.figure.colorbar(self.image, ax=self.axes, pad=0.03)
            self.colourbar.set_label("Count rate (counts s$^{-1}$)")
            self.figure.subplots_adjust(left=0.15, right=0.87, bottom=0.13, top=0.90)
        update_map_axes(self.axes, self.image, snapshot.summed_count_rate_cps, title=title)
        self.axes.set_facecolor("#fbfcfd"); self.colourbar.update_normal(self.image); self.canvas.draw_idle()
        self.plot_meta.setText(f"{snapshot.points_complete}/{snapshot.total_points} points")
        self.set_status(f"Elapsed: {snapshot.elapsed_s:.1f} s | Output: {snapshot.output_directory}")
    ...
    def _on_finished(self, result) -> None:
        ...
        if result.complete: self.progress.setValue(100)

    @pyqtSlot(str, str)
    def _on_failed(self, message: str, details: str) -> None:
        box = QMessageBox(self); box.setIcon(QMessageBox.Icon.Critical)
        box.setWindowTitle("Intensity-map acquisition failed"); box.setText(message)
        box.setDetailedText(details); box.exec()
        self.set_status("Acquisition failed. Partial checkpoint was retained.")

    @pyqtSlot()
    def _cleanup_worker(self) -> None:
        self.start_button.setEnabled(True); self.resume_button.setEnabled(True); self.stop_button.setEnabled(False)
    ...
    def set_laser_indicator(self, on: bool) -> None:
        if on: text, colour, border = "Laser output: ON", "#1f5a2a", "#5fd17a"
        else: text, colour, border = "Laser output: OFF", "#3a3a3a", "#666666"
        self.laser_light.setText(text)
        self.laser_light.setStyleSheet(
            f"QLabel {{ background-color: {colour}; color: white; border: 1px solid {border}; "
            "border-radius: 8px; padding: 6px; }"
        )

    def _toggle_laser_controls(self, enabled: bool) -> None:
        connected = self.laser is not None
        self.laser_address.setEnabled(enabled and not connected); self.laser_current.setEnabled(enabled)
        self.laser_connect.setEnabled(enabled); self.laser_on.setEnabled(enabled and connected)
        self.laser_off.setEnabled(enabled and connected); self.laser_set.setEnabled(enabled and connected)

    def _laser_error(self, title: str, exc: Exception) -> None:
        box = QMessageBox(self); box.setIcon(QMessageBox.Icon.Critical); box.setWindowTitle(title)
        box.setText(str(exc)); box.setDetailedText(traceback.format_exc()); box.exec()
    ...
    def connect_laser(self) -> None:
        ...
        try:
            if self.laser is not None: self.laser.close()
            laser = ITC4000(address, threshold_current=LASER_THRESHOLD_CURRENT_A, max_current=MAX_LASER_CURRENT_A)
            identity = laser.identify(); current = laser.get_current()
            diode_on = laser.get_diode_output(); tec_on = laser.get_tec_output()
            self.laser = laser; self.laser_current.setValue(current); self.set_laser_indicator(diode_on)
            self._toggle_laser_controls(True)
            self.set_status(f"Connected: {identity} | Current: {current:.3f} A | LD: {'ON' if diode_on else 'OFF'} | TEC: {'ON' if tec_on else 'OFF'}")
        except Exception as exc:  # noqa: BLE001
            self.laser = None; self.set_laser_indicator(False); self._toggle_laser_controls(True)
            self._laser_error("Laser connection failed", exc)

    def turn_laser_on(self) -> None:
        if self.laser is None: QMessageBox.warning(self, "No laser", "Connect the laser first."); return
        try:
            self.laser.enable(current=float(self.laser_current.value()))
            on = self.laser.get_diode_output(); current = self.laser.get_current()
            self.set_laser_indicator(on); self.set_status(f"Laser output {'ON' if on else 'OFF'}; current {current:.3f} A.")
        except Exception as exc: self.set_laser_indicator(False); self._laser_error("Laser enable failed", exc)

    def turn_laser_off(self) -> None:
        if self.laser is None: QMessageBox.warning(self, "No laser", "Connect the laser first."); return
        try:
            self.laser.disable(); on = self.laser.get_diode_output(); current = self.laser.get_current()
            self.laser_current.setValue(current); self.set_laser_indicator(on)
            self.set_status(f"Laser output {'ON' if on else 'OFF'}; current {current:.3f} A.")
        except Exception as exc: self._laser_error("Laser shutdown failed", exc)

    def set_laser_current(self) -> None:
        if self.laser is None: QMessageBox.warning(self, "No laser", "Connect the laser first."); return
        requested = float(self.laser_current.value())
        try:
            self.laser.set_current(requested); readback = self.laser.get_current()
            self.laser_current.setValue(readback)
            self.set_status(f"Requested {requested:.3f} A; controller readback {readback:.3f} A.")
        except Exception as exc: self._laser_error("Set current failed", exc)

    def closeEvent(self, event: Any) -> None:
        if self._thread is not None and self._thread.isRunning():
            QMessageBox.warning(self, "Acquisition in progress", "Stop the map and wait for checkpoint saving before closing.")
            event.ignore(); return
        if self.laser is not None:
            try: self.laser.disable(disable_tec=True)
            except Exception: logger.warning("Laser shutdown failed while closing the GUI")
            finally: self.laser.close(); self.laser = None
...
def main() -> None:
    ...
    window = IntensityMapWindow(); window.show()
    sys.exit(app.exec())
